...
4. **Canonical SSOT Modal Width (`max-w-sm`)**: Restored canonical `sm:max-w-sm` (384px) modal width with `p-5 sm:p-6` padding, and removed artificial inner squashing wrappers so form controls and OTP cells span `w-full` naturally with zero empty side margins.
