...
2. **Separated Country Code Badge & Overlap Fix**: Disentangled country code `+91` into a dedicated `h-14 w-16` badge (`bg-muted/40 border border-border/80 rounded-xl`) placed side-by-side with `<Input>`, completely eliminating text/help overlap.
3. **Ergonomic Field & Button Height (56px SSOT)**: Scaled mobile input, name input, OTP cells, and action buttons to standard comfortable `h-14` (56px) with generous touch padding. Fixed disabled button text contrast using `disabled:opacity-100 disabled:bg-muted disabled:text-muted-foreground/80 disabled:border disabled:border-border/60`.
4. **Canonical SSOT Modal Width (`sm:max-w-sm md:max-w-sm`)**: Locked dialog width to SSOT `sm:max-w-sm md:max-w-sm` (384px) with `p-5 sm:p-6` padding (overriding inherited `md:max-w-[540px]` from bottomSheet variant on desktop viewports), and eliminated artificial inner squashing wrappers so form controls and OTP cells span `w-full` naturally with zero empty lateral whitespace.
