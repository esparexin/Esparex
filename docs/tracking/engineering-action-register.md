...
10. **Side-by-Side CTA & Resend OTP Transition**: In the Mobile Step, `[ Back ]` sits side-by-side with `[ Send OTP ]`. In the OTP Step, the redundant bottom Back button was removed (the inline 'Change ✎' action on top handles mobile editing). The countdown timer ('Resend available in Xs') displays centered above while running, during which `[ Verify OTP ]` spans the full width. Once the timer completes, the `[ Resend OTP ]` outline button seamlessly appears beside `[ Verify OTP ]` as a side-by-side action.
