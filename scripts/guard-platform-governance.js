...
const MIGRATION_SHADOW_NAME_PATTERN = /(user-backend|admin-backend)\/scripts\/.*(migrate|remediate|repair|cleanup).+\.js$/;
...
const main = () => {
  ...
  for (const file of files) {
    ...
    const isMigration = file.includes('/migrations/');
  ...
  failIfAny('New migration shadow scripts detected in backend workspaces:', unknownMigrationShadow, failures);
  ...
  if (failures.length > 0) {
    ...
    console.error('\n💡 HINT: JS-based database mutations outside of (user-backend|admin-backend)/migrations/ bypass audit trails and lifecycle hooks.');
