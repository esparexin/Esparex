...
This decision record establishes a standardized **External System Integration Strategy** to guarantee domain isolation and reliable message delivery.
...
- **Adapter**: Resides in `core/adapters/`, wrapping the vendor SDK and implementing the port (e.g. `RazorpayAdapter`).

### 2. Anti-Corruption Layer (ACL)
Every adapter acts as an Anti-Corruption Layer. The adapter is responsible for:
- Translating external API responses and webhook payloads into pure domain entities or kernel primitives.
- Catching vendor-specific exceptions and translating them into standard kernel errors (`DomainError`).
...
2. A separate worker process (`services/worker/`) polls the outbox, publishes the events to the messaging queue, and marks them as processed.
This guarantees **at-least-once delivery** of cross-context integration events.
...
- Non-transient errors (e.g. invalid authentication, schema mismatches) must fail fast, trigger alerts, and bypass retries.
