...
|---|---|---|---|---|---|
| 1 | **Repository Topology** | `PASS WITH OBSERVATIONS` | 2026-07-13 | On structural change | Current topology (`core` and `shared` at root) is the accepted baseline for `v1.0` (`ADR-005`, `ADR-007`) and remains valid until a future restructuring is approved through an ADR. `apps/mobile/` is now fully integrated as a native React Native/Expo TypeScript workspace. |
...
|---|---|---|---|---|---|---|
| **EX-001** | `Exception` | S1 (Package Ownership) | `apps/mobile/` is not a registered npm workspace | Capacitor native shell wrapping `apps/web` via server URL (`https://esparex.in`). No independent TypeScript build pipeline; registering as workspace adds package overhead without tooling value. | Retired: apps/mobile is now fully integrated as a native React Native/Expo TypeScript workspace. | **RETIRED / RESOLVED** |
