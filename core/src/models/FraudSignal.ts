...
const FraudSignalSchema = new Schema<IFraudSignal>({
    ...
    createdAt: { type: Date, default: Date.now } // Auto-cleanup after 90 days
});
...
FraudSignalSchema.index({ createdAt: 1 }, { name: 'idx_fraudsignal_ttl_idx', expireAfterSeconds: 90 * 24 * 60 * 60 });
