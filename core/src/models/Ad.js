...
const AdSchema = new mongoose_1.Schema({
    ...
    status: {
        type: String,
        enum: adStatus_1.AD_STATUS_VALUES,
        default: adStatus_1.AD_STATUS.PENDING
    },
    ...
    timeline: [{
            status: { type: String, enum: [...adStatus_1.AD_STATUS_VALUES, 'deleted', 'restored', 'spotlight_expired'], required: true },
...
AdSchema.index({ status: 1, isDeleted: 1, expiresAt: 1, createdAt: -1 }, {
    ...
    partialFilterExpression: { status: adStatus_1.AD_STATUS.LIVE, isDeleted: false }
});
...
AdSchema.index({ status: 1, isSpotlight: 1, createdAt: -1 }, {
    ...
    partialFilterExpression: { status: adStatus_1.AD_STATUS.LIVE }
});
AdSchema.index({
    'location.country': 1,
    'location.state': 1,
...
AdSchema.index({ duplicateFingerprint: 1 }, {
    ...
    partialFilterExpression: {
        status: { $in: [adStatus_1.AD_STATUS.LIVE, adStatus_1.AD_STATUS.PENDING] },
