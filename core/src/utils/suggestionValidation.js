...
exports.validateBrandSuggestion = validateBrandSuggestion;
exports.validateModelSuggestion = validateModelSuggestion;
/**
 ...
 */
function cleanInput(input, allowedPattern) {
 ...
 */
function hasSpamCharacteristics(input, originalInput) {
    if (EMOJI_PATTERN.test(originalInput))
        return 'Emojis are not allowed.';
    if (URL_PATTERN.test(input) || URL_PATTERN.test(originalInput))
        return 'URLs and links are not allowed.';
    if (EMAIL_PATTERN.test(input) || EMAIL_PATTERN.test(originalInput))
        return 'Email addresses are not allowed.';
    if (PHONE_PATTERN.test(input) || PHONE_PATTERN.test(originalInput))
 ...
 */
function validateBrandSuggestion(input) {
    ...
    if (spamError)
        return { isValid: false, error: spamError, cleanName };
    // 3. Length Checks
    if (cleanName.length < 2)
        return { isValid: false, error: 'Brand name must be at least 2 characters long.', cleanName };
    if (cleanName.length > 40)
        return { isValid: false, error: 'Brand name cannot exceed 40 characters.', cleanName };
    // 4. Specific Rules
 ...
 */
function validateModelSuggestion(input) {
    if (!input || typeof input !== 'string') {
        return { isValid: false, error: 'Name is required.', cleanName: '' };
    }
    const originalInput = input;
    ...
    // 3. Length Checks
    if (cleanName.length < 2)
        return { isValid: false, error: 'Model name must be at least 2 characters long.', cleanName };
    if (cleanName.length > 50)
        return { isValid: false, error: 'Model name cannot exceed 50 characters.', cleanName };
    // 5. Auto Format
    cleanName = autoFormat(cleanName);
    return { isValid: true, cleanName };
}
