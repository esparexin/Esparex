...
const adminApproveListing = async (id, actorId, logFn, reviewVersion) => {
    ...
    const updated = await (0, StatusMutationService_1.mutateStatus)({
        domain: 'ad',
        entityId: id,
        toStatus: adStatus_1.AD_STATUS.LIVE,
    ...
    await logFn('LISTING_APPROVE', 'Ad', id, { status: adStatus_1.AD_STATUS.LIVE });
    return updated;
};
...
const adminRejectListing = async (id, actorId, rejectionReason, logFn) => {
    ...
    const updated = await (0, StatusMutationService_1.mutateStatus)({
        ...
        patch: {
            ...
            $push: {
                timeline: {
                    status: adStatus_1.AD_STATUS.REJECTED,
                    timestamp: new Date(),
                    reason: rejectionReason,
                },
            },
...
const adminDeactivateListing = async (id, actorId, logFn) => {
    validateListingId(id);
    const listing = await getListingForMutation(id);
    if (listing.status === adStatus_1.AD_STATUS.DEACTIVATED) {
        const currentListing = await (0, ListingModerationQueryService_1.getModerationListingById)(id);
        return { action: 'deactivated', listing: currentListing || { id, status: adStatus_1.AD_STATUS.DEACTIVATED, listingType: listing.listingType || 'ad' }, message: 'Listing is already deactivated' };
    }
    const updated = await (0, StatusMutationService_1.mutateStatus)({
        domain: 'ad',
        entityId: id,
        toStatus: adStatus_1.AD_STATUS.DEACTIVATED,
        actor: buildAdminActor(actorId),
        ...
        patch: {
            ...
            $push: {
                timeline: {
                    status: adStatus_1.AD_STATUS.DEACTIVATED,
                    timestamp: new Date(),
                    reason: 'Deactivated by moderation',
                },
            },
...
const adminExpireListing = async (id, actorId, logFn) => {
    ...
    const updated = await (0, StatusMutationService_1.mutateStatus)({
        domain: 'ad',
        entityId: id,
        toStatus: adStatus_1.AD_STATUS.EXPIRED,
        actor: buildAdminActor(actorId),
        ...
        patch: {
            ...
            $push: {
                timeline: {
                    status: adStatus_1.AD_STATUS.EXPIRED,
                    timestamp: new Date(),
                    reason: 'Expired by moderation',
                },
            },
...
const adminExtendListing = async (id, actorId, logFn) => {
    ...
    const now = new Date();
    const isExpired = listing.status === adStatus_1.AD_STATUS.EXPIRED;
    let updated;
    if (isExpired) {
