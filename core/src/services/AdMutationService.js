...
const updateAd = async (adId, data, context, externalSession) => {
    return (0, AdUpdateService_1.updateAdLogic)(adId, data, context, externalSession);
};
...
const promoteAd = async (id, days = 7, type = 'spotlight_hp', userId, isAdmin = false) => {
    return (0, AdPromotionService_1.promoteAdLogic)(id, days, type, userId, isAdmin);
};
exports.promoteAd = promoteAd;
const repostAd = async (id, userId) => {
    return (0, AdRepostService_1.repostAdLogic)(id, userId);
};
...
const findOwnedService = async (id, userId, listingType, fetchFull) => {
    ...
    if (fetchFull) {
        return Ad_1.default.findOne({ _id: objectId, listingType, sellerId: userId });
    }
    return Ad_1.default.findOne({ _id: objectId, listingType, sellerId: userId, isDeleted: { $ne: true } }).select('status');
};
