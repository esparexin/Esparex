...
const canBypassInvalidTransition = (params) => {
    ...
    if (!isListingLifecycleDomain(params.resolvedDomain))
        return false;
    if (toLower(params.toStatus) !== listingStatus_1.LISTING_STATUS.DEACTIVATED)
        return false;
...
const mutateStatus = async (request) => {
    ...
    try {
        ...
        if (domain === 'ad') {
            ...
            Promise.resolve().then(() => __importStar(require('@core/utils/redisCache'))).then(({ invalidatePublicAdCache, invalidateAdFeedCaches }) => {
                return Promise.all([
                    invalidatePublicAdCache(entityId.toString()),
                    invalidateAdFeedCaches()
                ]);
            }).catch((cacheErr) => {
                logger_1.default.error('Failed to bust cache during status mutation', { adId: entityId.toString(), cacheErr });
            });
...
const mutateStatusesBulk = async (domain, entityIds, toStatus, actor, reason) => {
    ...
    if (domain === 'ad' && toStatus === listingStatus_1.LISTING_STATUS.EXPIRED) {
