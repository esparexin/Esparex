...
const getServiceAnalyticsStats = async () => {
    const [totalServices, pendingServices, activeServices] = await Promise.all([
        ...
        adServiceBase_1.Ad.countDocuments({ listingType: listingType_1.LISTING_TYPE.SERVICE, status: adServiceBase_1.LISTING_STATUS.LIVE }),
    ]);
...
const getAdCounts = async (filters, options = {}) => {
    ...
    const [live, pending, sold, rejected, expired, deactivated, total] = await Promise.all([
        adServiceBase_1.Ad.countDocuments({ ...query, status: adServiceBase_1.LISTING_STATUS.LIVE }),
        adServiceBase_1.Ad.countDocuments({ ...query, status: adServiceBase_1.LISTING_STATUS.PENDING }),
        adServiceBase_1.Ad.countDocuments({ ...query, status: adServiceBase_1.LISTING_STATUS.SOLD }),
        adServiceBase_1.Ad.countDocuments({ ...query, status: adServiceBase_1.LISTING_STATUS.REJECTED }),
        adServiceBase_1.Ad.countDocuments({ ...query, status: adServiceBase_1.LISTING_STATUS.EXPIRED }),
        adServiceBase_1.Ad.countDocuments({ ...query, status: adServiceBase_1.LISTING_STATUS.DEACTIVATED }),
