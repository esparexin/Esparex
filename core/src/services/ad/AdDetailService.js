...
const getReportedAdsAggregation = async (filters, pagination) => {
    ...
    const pipeline = [
        ...
        {
            $addFields: {
                isAutoHidden: { $eq: ['$adDetails.moderationStatus', adServiceBase_1.LISTING_STATUS.REJECTED] }
            }
        },
        {
            $sort: {
...
const getAdSuggestions = async (q, limit = 10) => {
    ...
    const docs = await adServiceBase_1.Ad.find({ title: regex, status: adServiceBase_1.LISTING_STATUS.LIVE, isDeleted: { $ne: true } }, { title: 1 }).limit(limit).lean();
...
const getAdsByStatus = async (status, pagination) => {
    ...
    const [data, total] = await Promise.all([
        adServiceBase_1.Ad.find({ status: status, isDeleted: { $ne: true } })
            .sort({ createdAt: 1 })
            .skip(skip)
            .limit(limit)
            .lean(),
        adServiceBase_1.Ad.countDocuments({ status: status, isDeleted: { $ne: true } })
    ]);
