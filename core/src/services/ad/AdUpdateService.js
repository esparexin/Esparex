...
const updateAdLogic = async (adId, data, context, externalSession) => {
    ...
    try {
        ...
        const executeUpdate = async () => {
            ...
            if (context.actor === 'USER' && (ad.status === listingStatus_1.LISTING_STATUS.LIVE || ad.status === listingStatus_1.LISTING_STATUS.PENDING)) {
            ...
            const requiresReviewTransition = context.actor === 'USER' && ad.status === listingStatus_1.LISTING_STATUS.LIVE;
            ...
            if (updatedAd && requiresReviewTransition) {
                updatedAd = await (0, StatusMutationService_1.mutateStatus)({
                    domain: 'ad',
                    entityId: id,
                    toStatus: listingStatus_1.LISTING_STATUS.PENDING,
                    actor: {
                    ...
                    patch: {
                        ...
                        $push: {
                            timeline: {
                                status: listingStatus_1.LISTING_STATUS.PENDING,
                                timestamp: new Date(),
                                reason: 'Re-submitted for review after edit',
                            },
                        },
