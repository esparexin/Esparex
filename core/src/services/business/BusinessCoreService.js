...
const getBusinessStats = async (userId) => {
    const [totalServices, approvedServices, pendingServices] = await Promise.all([
        ...
        Ad_1.default.countDocuments({ sellerId: userId, listingType: 'service', status: adStatus_1.AD_STATUS.LIVE }),
        Ad_1.default.countDocuments({ sellerId: userId, listingType: 'service', status: adStatus_1.AD_STATUS.PENDING }),
    ]);
