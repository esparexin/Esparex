...
const cascadeExpireBusinessListings = async (businessId, actor, reason) => {
    ...
    const listings = await Ad_1.default.find({ businessId: normalizedBusinessId, status: { $ne: adStatus_1.AD_STATUS.EXPIRED } }).select('_id listingType');
...
const getAdminBusinessAccountsData = async (params) => {
