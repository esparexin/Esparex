        ...
        Ad_1.default.aggregate([
            {
                $facet: {
                    ...
                    pendingAds: [{ $match: { listingType: listingType_1.LISTING_TYPE.AD, status: adStatus_1.AD_STATUS.PENDING } }, { $count: "count" }],
                    ...
                    pendingServices: [{ $match: { listingType: listingType_1.LISTING_TYPE.SERVICE, status: adStatus_1.AD_STATUS.PENDING } }, { $count: "count" }],
                    rejectedServices: [{ $match: { listingType: listingType_1.LISTING_TYPE.SERVICE, status: adStatus_1.AD_STATUS.REJECTED } }, { $count: "count" }],
                    ...
                    pendingSpareParts: [{ $match: { listingType: listingType_1.LISTING_TYPE.SPARE_PART, status: adStatus_1.AD_STATUS.PENDING } }, { $count: "count" }]
                }
            }
        ]),
        ...
        Ad_1.default.aggregate([
            {
                $facet: {
                    ...
                    pending: [{ $match: { listingType: listingType_1.LISTING_TYPE.AD, status: adStatus_1.AD_STATUS.PENDING } }, { $count: "count" }]
                }
            }
        ]),
...
const getLocationAnalyticsRawData = async (params) => {
    ...
    const [totalLocations, totalAds, totalUsers, adsByLocationAgg, monthlyAds, monthlyUsers, monthlyLocs, topHotZonesRaw] = await Promise.all([
        ...
        Ad_1.default.countDocuments(buildScopedAdQuery({ status: adStatus_1.AD_STATUS.LIVE })),
        User_1.default.countDocuments(buildScopedUserQuery({ status: userStatus_1.USER_STATUS.LIVE })),
        Ad_1.default.aggregate([
            {
                $match: {
                    ...buildScopedAdQuery({ status: adStatus_1.AD_STATUS.LIVE }),
