...
const enforceLifecycleMutationPolicy = (input) => {
    ...
    if (action === 'repost'
        && toLower(input.fromStatus) === listingStatus_1.LISTING_STATUS.EXPIRED
        && toLower(input.toStatus) === listingStatus_1.LISTING_STATUS.LIVE) {
    ...
    if (!isListingDomain(input.domain))
        return;
    if (toLower(input.toStatus) !== listingStatus_1.LISTING_STATUS.LIVE)
        return;
