...
// ─────────────────────────────────────────────────
const s3_2 = require("@core/utils/s3");
/**
 * Generates a presigned URL for direct S3 upload from the client.
 */
const getUploadPresignedUrl = async (userId, fileName, fileType) => {
    const key = `temp/uploads/${userId}/${Date.now()}-${fileName}`;
    return (0, s3_2.generatePresignedUploadUrl)(key, fileType);
};
