...
exports.FeedDecisionEngine = void 0;
...
class FeedDecisionEngine {
    static async getFallbackFeed(input, excludeIds, limitNeeded, categoryId) {
        let currentStage = 'RADIUS';
        let appliedRadiusKm = 10;
        const mergedAds = [];
        const seenIds = new Set(excludeIds);
        const fetchBatch = async (matchStage, stageName, radius = 0) => {
            if (mergedAds.length >= limitNeeded)
                return;
            const visibilityFilter = (0, FeedVisibilityGuard_1.buildPublicAdFilter)();
            const pipeline = [
                { $match: { ...matchStage, ...visibilityFilter, _id: { $nin: Array.from(seenIds).map(id => new mongoose_1.default.Types.ObjectId(id)) } } },
                { $sort: { createdAt: -1 } },
                { $limit: limitNeeded - mergedAds.length },
            ];
            const results = await Ad_1.default.aggregate(pipeline);
        ...
        const baseMatch = {};
        if (categoryId) {
            baseMatch.categoryId = mongoose_1.default.Types.ObjectId.isValid(categoryId) ? new mongoose_1.default.Types.ObjectId(categoryId) : categoryId;
        }
        ...
        if (input.locationId && mongoose_1.default.Types.ObjectId.isValid(input.locationId) && mergedAds.length < limitNeeded) {
            currentStage = 'CITY';
            const locObjId = new mongoose_1.default.Types.ObjectId(input.locationId);
            await fetchBatch({ ...baseMatch, $or: [{ locationPath: locObjId }, { 'location.locationId': locObjId }] }, 'CITY');
        }
        // Stage 2 fallback: City string regex (anchored + escaped, uses ad_city_status_freshness_idx)
            ...
            currentStage = 'CITY';
            await fetchBatch({ ...baseMatch, 'location.city': new RegExp(`^${escapeRegex(input.city)}$`, 'i') }, 'CITY');
        }
        ...
        if (input.state && mergedAds.length < limitNeeded) {
            currentStage = 'STATE';
            await fetchBatch({ ...baseMatch, 'location.state': new RegExp(`^${escapeRegex(input.state)}$`, 'i') }, 'STATE');
        }
        ...
        if (input.state && mergedAds.length < limitNeeded) {
            ...
            if (neighbors.length > 0) {
                ...
                await fetchBatch({ ...baseMatch, 'location.state': { $in: neighbors.map(n => new RegExp(`^${escapeRegex(n)}$`, 'i')) } }, 'REGIONAL');
            }
        }
        ...
        if (env_1.env.FEED_DEBUG) {
            logger_1.default.debug(`[FeedDecisionEngine] Fallback triggered. Stage: ${currentStage}, Recovered: ${mergedAds.length}`);
        }
        return {
            ads: mergedAds,
            meta: {
...
exports.FeedDecisionEngine = FeedDecisionEngine;
//# sourceMappingURL=FeedDecisionEngine.js.map