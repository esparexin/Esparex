...
exports.scanHierarchyIntegrity = scanHierarchyIntegrity;
exports.getHierarchyTree = getHierarchyTree;
exports.repairHierarchy = repairHierarchy;
exports.activateValidRecords = activateValidRecords;
...
const buildActivateOps = (docs, predicate) => docs
    .filter(predicate)
 ...
 */
async function scanHierarchyIntegrity() {
    const issues = [];
    const validCatIds = new Set(await (0, CatalogValidationService_1.getActiveCategoryIds)());
    // ── Brands ──────────────────────────────────────────────────────────────
    const allBrands = await Brand_1.default.find({ isDeleted: { $ne: true } })
    ...
    // ── Models ───────────────────────────────────────────────────────────────
    const validBrandIds = new Set((await Brand_1.default.find({ isDeleted: { $ne: true }, isActive: true })
        .select('_id').lean()).map((b) => String(b._id)));
    const allModels = await Model_1.default.find({ isDeleted: { $ne: true } })
        .select('_id brandId')
        .lean();
    ...
    // ── Spare Parts ──────────────────────────────────────────────────────────
    const allParts = await SparePart_1.default.find({ isDeleted: { $ne: true } })
        .select('_id categoryIds')
        .lean();
    let orphanedParts = 0;
    let staleCategoryParts = 0;
    for (const part of allParts) {
        const cats = (part.categoryIds ?? []).map(String);
        if (cats.length === 0) {
            orphanedParts++;
    ...
    // ── Screen Sizes ─────────────────────────────────────────────────────────
    const allSizes = await ScreenSize_1.default.find({ isDeleted: { $ne: true } })
...
async function getHierarchyTree() {
    const [categories, brands, models] = await Promise.all([
        Category_1.default.find({ isDeleted: { $ne: true } })
            .select('_id name slug listingType hasScreenSizes isActive')
            .sort({ name: 1 })
            .lean(),
        Brand_1.default.find({ isDeleted: { $ne: true } })
            .select('_id name categoryIds isActive status')
            .sort({ name: 1 })
            .lean(),
        Model_1.default.find({ isDeleted: { $ne: true } })
    ...
    const categoriesTree = categories.map((category) => {
        ...
        const categoryBrands = brands
            .filter((brand) => (brand.categoryIds ?? []).some((id) => String(id) === categoryId))
            .map((brand) => {
            const brandModels = (modelsByBrand.get(String(brand._id)) ?? []).filter((model) => {
                const mappedCategoryIds = (model.categoryIds ?? []).map((id) => String(id));
...
async function repairHierarchy() {
    ...
    const categories = await Category_1.default.find({ isDeleted: { $ne: true }, isActive: true })
    ...
    const orphanBrands = await Brand_1.default.find({
        isDeleted: { $ne: true },
        $or: [
            { categoryIds: null },
    ...
    const brandRepairOps = [];
    const brandOrphanOps = [];
    for (const brand of orphanBrands) {
        const nameLower = (brand.name ?? '').toLowerCase().trim();
        const matchedCatId = catByName.get(nameLower) ??
    ...
    if (brandRepairOps.length)
        await Brand_1.default.bulkWrite(brandRepairOps, { ordered: false });
    if (brandOrphanOps.length)
        await Brand_1.default.bulkWrite(brandOrphanOps, { ordered: false });
    // ── 2. SpareParts: repair / prune categoryIds[] ────────────────────────────
    const brandCatMap = new Map((await Brand_1.default.find({
        isDeleted: { $ne: true },
        isActive: true,
        categoryIds: { $exists: true, $not: { $size: 0 } },
    }).select('_id categoryIds').lean())
        .map((b) => [String(b._id), b.categoryIds?.[0] || null]));
    const emptyParts = await SparePart_1.default.find({
        isDeleted: { $ne: true },
        $or: [
            { categoryIds: { $exists: false } },
            { categoryIds: null },
    ...
    const allActiveParts = await SparePart_1.default.find({
        isDeleted: { $ne: true },
        categoryIds: { $exists: true, $not: { $size: 0 } },
    }).select('_id categoryIds').lean();
    const pruneOps = [];
    for (const part of allActiveParts) {
        ...
        if (validCats.length === 0) {
            ...
            pruneOps.push({
                updateOne: { filter: { _id: part._id }, update: { $set: { categoryIds: validCats.map((id) => new mongoose_1.default.Types.ObjectId(id)) } } },
            });
        }
    }
    if (partRepairOps.length)
        await SparePart_1.default.bulkWrite(partRepairOps, { ordered: false });
    if (partOrphanOps.length)
        await SparePart_1.default.bulkWrite(partOrphanOps, { ordered: false });
    if (pruneOps.length)
        await SparePart_1.default.bulkWrite(pruneOps, { ordered: false });
    // ── 3. ScreenSizes: deactivate orphans ────────────────────────────────────
    const orphanSizes = await ScreenSize_1.default.find({
        isDeleted: { $ne: true },
        $or: [
            { categoryId: null },
    ...
    const staleSizes = await ScreenSize_1.default.find({ isDeleted: { $ne: true } })
        .select('_id categoryId').lean();
    const sizeDeactivateOps = [];
    ...
    void orphanSizes;
    if (sizeDeactivateOps.length)
        await ScreenSize_1.default.bulkWrite(sizeDeactivateOps, { ordered: false });
    return summary;
}
 ...
 */
async function activateValidRecords() {
    const validCatIds = new Set(await (0, CatalogValidationService_1.getActiveCategoryIds)());
    // Brands: status=active + valid categoryId + not flagged
    const inactiveBrands = await Brand_1.default.find({
        isDeleted: { $ne: true },
        isActive: false,
    ...
    const activateBrandOps = buildActivateOps(inactiveBrands, (b) => (b.categoryIds ?? []).some((id) => validCatIds.has(String(id))));
    if (activateBrandOps.length)
        await Brand_1.default.bulkWrite(activateBrandOps, { ordered: false });
    // SpareParts: inactive + valid categoryIds
    const inactiveParts = await SparePart_1.default.find({
        isDeleted: { $ne: true },
        isActive: false,
        categoryIds: { $exists: true, $not: { $size: 0 } },
    }).select('_id categoryIds').lean();
    const activatePartOps = buildActivateOps(inactiveParts, (p) => (p.categoryIds ?? []).some((id) => validCatIds.has(String(id))));
    if (activatePartOps.length)
        await SparePart_1.default.bulkWrite(activatePartOps, { ordered: false });
    // ScreenSizes: valid categoryId + inactive
    const inactiveSizes = await ScreenSize_1.default.find({
        isDeleted: { $ne: true },
        isActive: false,
    }).select('_id categoryId').lean();
    const activateSizeOps = buildActivateOps(inactiveSizes, (ss) => !!(ss.categoryId && validCatIds.has(String(ss.categoryId))));
    if (activateSizeOps.length)
        await ScreenSize_1.default.bulkWrite(activateSizeOps, { ordered: false });
    return {
...
//# sourceMappingURL=CatalogHierarchyService.js.map