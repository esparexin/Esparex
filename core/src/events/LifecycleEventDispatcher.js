...
const PROTECTED_EVENT_SOURCE_MARKERS = {
...
const validateEventSource = (eventName) => {
    const allowedMarkers = PROTECTED_EVENT_SOURCE_MARKERS[eventName];
    if (!allowedMarkers || allowedMarkers.length === 0)
        return null;
...
class LifecycleEventDispatcher {
     ...
     */
    on(eventName, handler, listenerName) {
        this.emitter.on(eventName, (payload) => {
            void Promise.resolve(handler(payload)).catch(error => {
                logger_1.default.error(`[LifecycleEventDispatcher] Error in listener '${listenerName || 'Anonymous'}' for event '${eventName}':`, {
    ...
    async dispatch(eventName, payload) {
        ...
        if (sourceViolation) {
            logger_1.default.error('[LifecycleEventDispatcher] Blocked lifecycle event emit from non-SSOT source.', {
                eventName,
                allowedMarkers: sourceViolation.allowedMarkers,
                payload,
            });
            if (env_1.env.NODE_ENV !== 'production') {
                logger_1.default.error('[LifecycleEventDispatcher][PANIC_DEV] Invalid lifecycle emit stack detected.', {
                    eventName,
        ...
        logger_1.default.debug(`[LifecycleEventDispatcher] Dispatching ${eventName}`, { payload });
        ...
        setImmediate(() => {
            try {
                ...
                logger_1.default.error(`[LifecycleEventDispatcher] Failed to emit ${eventName}:`, {
                    error: error instanceof Error ? error.message : String(error)
                });
            }
        });
...
exports.lifecycleEvents = new LifecycleEventDispatcher();
//# sourceMappingURL=LifecycleEventDispatcher.js.map