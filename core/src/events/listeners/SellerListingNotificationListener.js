...
exports.registerSellerListingNotificationListener = void 0;
...
const FRONTEND_URL = (0, appUrl_1.getFrontendAppUrl)();
...
async function getSellerEmail(listingId) {
    try {
        const listing = await Ad_1.default.findById(listingId).select('sellerId title listingType').lean();
        if (!listing)
            return null;
        const user = await User_1.default.findById(listing.sellerId).select('email name').lean();
        if (!user?.email)
            return null;
        return { email: user.email, name: user.name || 'Seller', title: listing.title || 'Your listing', listingType: listing.listingType || 'ad' };
    }
    catch (e) {
        logger_1.default.warn('[SellerNotification] Failed to resolve seller email', { listingId, error: String(e) });
        return null;
    }
}
const registerSellerListingNotificationListener = () => {
    // Approval
    LifecycleEventDispatcher_1.lifecycleEvents.on('listing.approved', async (payload) => {
        try {
            ...
            `;
            await EmailService_1.emailService.sendEmail(info.email, subject, html);
            logger_1.default.info('[SellerNotification] Approval email sent', { listingId: payload.listingId, listingType: info.listingType });
        }
        catch (e) {
            logger_1.default.error('[SellerNotification] Failed to send approval email', { error: String(e), listingId: payload.listingId });
        }
    }, 'SellerNotification_Approved');
    // Rejection
    LifecycleEventDispatcher_1.lifecycleEvents.on('listing.rejected', async (payload) => {
        try {
            const info = await getSellerEmail(payload.listingId);
            if (!info)
                return;
            ...
            `;
            await EmailService_1.emailService.sendEmail(info.email, subject, html);
            logger_1.default.info('[SellerNotification] Rejection email sent', { listingId: payload.listingId });
        }
        catch (e) {
            logger_1.default.error('[SellerNotification] Failed to send rejection email', { error: String(e), listingId: payload.listingId });
        }
    }, 'SellerNotification_Rejected');
    // Expiry (bulk)
    LifecycleEventDispatcher_1.lifecycleEvents.on('listing.expired.bulk', async (payload) => {
        ...
        for (const listingId of payload.listingIds.slice(0, 50)) {
            try {
                ...
                logger_1.default.error('[SellerNotification] Failed to send expiry email', { error: String(e), listingId });
            }
        }
        logger_1.default.info('[SellerNotification] Expiry emails processed', { count: Math.min(payload.listingIds.length, 50) });
    }, 'SellerNotification_Expired');
    logger_1.default.info('[SellerListingNotification] Listener registered successfully.');
};
exports.registerSellerListingNotificationListener = registerSellerListingNotificationListener;
//# sourceMappingURL=SellerListingNotificationListener.js.map