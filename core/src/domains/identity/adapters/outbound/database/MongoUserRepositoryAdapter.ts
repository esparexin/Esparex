...
export class MongoUserRepositoryAdapter implements UserRepositoryPort {
    ...
    public async updateUser(id: string, updates: Record<string, unknown>): Promise<Record<string, unknown> | null> {
        ...
        ).select('-password').lean<Record<string, unknown> | null>();
    }

    public async removeUserFcmToken(userId: string | unknown, token: string): Promise<void> {
    ...
    public async getUserById(userId: string): Promise<Record<string, unknown> | null> {
        const safeId = typeof userId === 'string' ? userId : String(userId);
        const user = await User.findById(safeId).lean<Record<string, unknown> | null>();
    ...
    public async getUserWithBusiness(userId: string): Promise<{ user: Record<string, unknown> | null; business: Record<string, unknown> | null }> {
        ...
        const [user, business] = await Promise.all([
            User.findById(safeId).select('-password -salt').lean<Record<string, unknown> | null>(),
            Business.findOne({ userId: String(userId) }).lean<Record<string, unknown> | null>(),
        ]);
        if (user && user.role) user.role = normalizeRole(user.role as string);
        return { user, business };
    }

    public async getUserPhoneVerification(userId: string): Promise<UserPhoneVerificationData | null> {
        const safeId = typeof userId === 'string' ? userId : String(userId);
        return await User.findById(safeId).select('isPhoneVerified mobile').lean<UserPhoneVerificationData | null>();
    }

    public async findUserByEmail(email: string): Promise<Record<string, unknown> | null> {
        const safeEmail = typeof email === 'string' ? email : String(email ?? '');
        return await User.findOne({ email: safeEmail }).lean<Record<string, unknown> | null>();
    }

    public async getUserAvatarById(userId: string): Promise<UserAvatarData | null> {
        const safeId = typeof userId === 'string' ? userId : String(userId);
        return await User.findById(safeId).select('avatar').lean<UserAvatarData | null>();
    }
    ...
    public async blockUserById(blockerId: string, blockedUserId: string): Promise<unknown> {
