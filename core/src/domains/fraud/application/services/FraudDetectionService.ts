...
export const analyzeFraudRisk = async (context: FraudContext): Promise<FraudDecision> => {
    ...
    // 6. Record Fraud Signal & Persist Current Score Snapshot
    try {
        if (context.userId) {
        ...
        logger.error('Failed to log fraud signal/score snapshot', { error: err instanceof Error ? err.message : String(err) });
    }
