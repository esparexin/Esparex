...
interface BrowseSparePartsResultsProps {
  listings: SparePartListing[];
  total: number;
  sort: SortOption;
  view: "grid" | "list";
  loading: boolean;
...
export default function BrowseSparePartsResults({
  listings,
  total,
  sort,
  view,
  ...
  onReset,
  onLoadMore,
}: BrowseSparePartsResultsProps) {
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-6 lg:px-8 space-y-4">
      <SearchResultsHeader
        total={loading && listings.length === 0 ? 0 : total}
        sort={sort}
        view={view}
      ...
      {loading && listings.length === 0 && !error && <GridSkeleton />}
      ...
      {!loading && !error && listings.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-500 max-w-xs mb-6">
            {query ? `No spare parts matching "${query}".` : "No spare parts available in this area yet."}
          </p>
      ...
      {listings.length > 0 && (
        ...
        >
          {listings.map((listing) => (
            <BrowseSparePartsCard key={listing.id} listing={listing} />
          ))}
        </div>
      )}
