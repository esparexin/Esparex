...
export const saveAd = async (req: Request, res: Response) => {
    try {
        ...
        let safeAdId: string;
        try {
            safeAdId = validateObjectIdOrThrow('adId', finalAdId);
        } catch {
            return sendErrorResponse(req, res, 400, 'Invalid Ad ID format');
        }

        const result = await saveAdService(userId, safeAdId);
...
export const unsaveAd = async (req: Request, res: Response) => {
    try {
        ...
        let safeAdId: string;
        try {
            safeAdId = validateObjectIdOrThrow('adId', adId);
        } catch {
            return sendErrorResponse(req, res, 400, 'Invalid Ad ID format');
        }

        await unsaveAdService(userId, safeAdId);
