...
export const createServiceMutation = async ({
    ...
    const processedImages = await processServiceImages(safeBody.images, `services/${serviceId.toString()}`);
    if (processedImages.length > 0) {
        safeBody.images = processedImages;
    }

    const listingQualityScore = calculateServiceQuality(
        safeBody,
        business as unknown as Record<string, unknown>
    );

    return ListingMutationService.executeCreationTransaction({
