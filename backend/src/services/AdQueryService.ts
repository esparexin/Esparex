...
export const getPublicAdById = async (adId: string, viewer?: PublicAdViewer) => {
    ...
    const viewerObjectId = (
        viewer?.userId && mongoose.Types.ObjectId.isValid(viewer.userId)
    )
    ...
    const canUsePublicCache = !viewerObjectId && !isAdminViewer;
    const publicCacheKey = canUsePublicCache ? `ad:public:${adId}` : null;

    if (publicCacheKey) {
        const cachedAd = await getCache<Record<string, unknown>>(publicCacheKey);
        if (cachedAd) {
            return cachedAd;
        }
    }
    ...
    if (!isAdminViewer) {
        const publicVisibilityFilter = buildPublicAdFilter();
        if (viewerObjectId) {
            // Enforce explicit grouping:
            // _id AND isDeleted AND (publicVisibility OR owner)
            visibilityFilter = {
                $and: [
                    { _id: objectId },
                    { isDeleted: { $ne: true } },
                    {
                        $or: [
                            ...
                            { sellerId: viewerObjectId }
                        ]
                    }
                ]
            };
        } else {
            Object.assign(visibilityFilter, publicVisibilityFilter);
        }
    }

    try {
        const ad = await Ad.findOne(visibilityFilter)
            .populate('sellerId', 'name avatar isVerified role trustScore')
            // Category is mastered on the admin connection; use explicit model for safe cross-connection populate.
        ...
        const { password, otp, otpExpiry, ...safeAd } = ad as any;
        const sanitizedAd = normalizeAdImagesForResponse(safeAd);
        if (publicCacheKey) {
            await setCache(publicCacheKey, sanitizedAd, AD_DETAIL_CACHE_TTL_SECONDS);
        }
        return sanitizedAd;
    } catch (error: unknown) {
        logger.error('Ad retrieval failed', {
            adId,
