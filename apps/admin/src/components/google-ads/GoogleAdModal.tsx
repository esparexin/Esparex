...
export function GoogleAdModal({ isOpen, onClose, onSave, editingPlacement }: GoogleAdModalProps) {
    ...
    const [location, setLocation] = useState<string>(AD_PLACEMENT_LOCATION.HOMEPAGE_HERO);
    const [format, setFormat] = useState<string>(AD_FORMAT.LEADERBOARD_728x90);
    const [status, setStatus] = useState<string>(GOOGLE_AD_STATUS.ACTIVE);
    const [fallbackStrategy, setFallbackStrategy] = useState<string>(AD_FALLBACK_STRATEGY.COLLAPSE);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const success = await onSave({
                ...
                fallbackStrategy: fallbackStrategy as any,
                viewports,
            });
            if (success) onClose();
        } finally {
    ...
    const toggleViewport = (vp: "desktop" | "tablet" | "mobile") => {
        setViewports((prev) =>
            prev.includes(vp) ? prev.filter((item) => item !== vp) : [...prev, vp]
        );
    };
