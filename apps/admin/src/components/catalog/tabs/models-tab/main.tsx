        ...
        <>
            <CatalogPageTemplate<Model, ModelFormData>
                ...
                generateColumns={(openEditModal) => [
                    ...
                    { header: "Approval State", cell: (m) => { const ls = deriveCatalogLifecycleStatus(m); return <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${ls === 'active' ? "bg-emerald-100 text-emerald-700" : ls === 'pending' ? "bg-amber-100 text-amber-700" : ls === 'rejected' ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-700"}`}>{ls}</span>; } },
