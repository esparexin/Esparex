...
export interface OtpFlowState {
    ...
    requestOtp: (mobile: string, fallbackMessage?: string) => Promise<void>;
    handleResendOtp: (mobile: string) => Promise<void>;
    resetToMobileStep: (mobile?: string) => void;
    verifyOtpCode: (mobile: string, otpValue: string, newUserName?: string) => Promise<void>;
}
...
export function useOtpFlow(
    ...
    }, [step, rateLimit, formatRateLimitMessage]);
    ...
    const resetOtpSession = useCallback(() => {
        ...
        clearRateLimit();
    }, [clearRateLimit, setLockUntilMs, setResendAvailableAtMs]);

    const resetToMobileStep = useCallback((mobile?: string) => {
        if (isOtpStep && mobile && validateIndianMobile(mobile)) {
        ...
        setExistingUserName("");
        sendMachine("RESET");
    }, [isOtpStep, resetOtpSession, sendMachine]);
    ...
    const requestOtp = useCallback(
        async (mobile: string, fallbackMessage = "Failed to send OTP. Please try again.") => {
            ...
            try {
                ...
                const message = mapAuthError(err, fallbackMessage);
        ...
        [applyLockedState, applyRateLimitState, transitionToOtpStep, sendMachine]
    );

    const verifyOtpCode = useCallback(
        async (mobile: string, otpValue: string, newUserName?: string) => {
        ...
        [applyLockedState, applyOtpExpiredState, applyRateLimitState, clearRateLimit, isBlocked, isLocked, isVerifyRateLimited, onLoginSuccess, step, updateUser, sendMachine, setResendAvailableAtMs]
    );

    const handleResendOtp = useCallback(async (mobile: string) => {
        if (isSendingOTP || isVerifying || isBlocked || isLocked || isSendRateLimited) return;
        await requestOtp(mobile, "Resend failed. Please try again.");
