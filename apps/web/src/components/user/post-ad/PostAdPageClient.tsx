...
function PostAdPageBackdrop() {
    return (
        <div className="fixed inset-0 overflow-hidden bg-muted/90 pointer-events-none select-none" inert>
            {/* Header Shell */}
            <header className="w-full bg-card border-b border-border/80 px-4 py-3 sm:px-6 flex items-center justify-between shadow-sm">
                ...
                <div className="hidden sm:flex items-center gap-2 max-w-md w-full mx-8">
                    <div className="w-full h-9 rounded-full bg-muted border border-border px-4 flex items-center text-muted-foreground text-body">
            ...
            <main className="max-w-6xl mx-auto px-4 py-6 sm:px-6 space-y-6">
                ...
                <div className="space-y-3">
                    <div className="w-36 h-5 rounded bg-muted-foreground/30" />
                ...
                <div className="space-y-3 pt-2">
                    <div className="w-44 h-5 rounded bg-muted-foreground/30" />
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {[1, 2, 3, 4].map((item) => (
                            <div key={item} className="rounded-2xl border border-border bg-card p-3 space-y-3 shadow-sm">
                                <div className="w-full h-36 rounded-xl bg-muted flex items-center justify-center text-muted-foreground/50 text-caption font-medium">
                                    Product Image
                                </div>
