...
export const ActiveSubscriptionCard: React.FC<ActiveSubscriptionCardProps> = ({
  ...
  const isExpired = Boolean(
    ...
    (subscription?.endDate && new Date(subscription.endDate).getTime() < currentTime)
  );

  // Do not render banner box for free tiers or expired packs — eliminates noise and clutter
  if (!subscription || isExpired || subscription.category === 'FREE') {
    return null;
  }
