    ...
    return (
        <div className="flex flex-col gap-1.5 scroll-mt-[calc(6rem+env(safe-area-inset-top,0px))]">
            ...
            <Input
                ...
                maxLength={15}
                {...register("gstin")}
                className={`h-11 rounded-xl bg-card border-border px-3.5 text-body-lg md:text-body font-normal text-foreground placeholder:text-caption sm:placeholder:text-body placeholder:text-foreground-subtle uppercase placeholder:normal-case scroll-mt-[calc(7.5rem+env(safe-area-inset-top,0px))] ${gstinError ? "border-destructive" : ""}`}
