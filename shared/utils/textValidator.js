...
exports.TEXT_QUALITY_RULES = exports.GIBBERISH_PATTERNS = exports.ALL_BANNED_WORDS = exports.BANNED_WORDS = void 0;
exports.validateText = validateText;
exports.isTextValid = isTextValid;
...
function checkBannedWords(text) {
    ...
    for (const [category, words] of Object.entries(bannedWords_1.BANNED_WORDS)) {
        for (const word of words) {
            // Use word boundary matching for accuracy
            const regex = new RegExp(`\\b${escapeRegex(word)}\\b`, 'i');
            if (regex.test(lowerText)) {
                const isHardReject = bannedWords_1.HARD_REJECT_CATEGORIES.includes(category);
                const isModeration = bannedWords_1.MODERATION_CATEGORIES.includes(category);
                issues.push({
                    type: 'banned_word',
...
function checkGibberish(text) {
    ...
    if (bannedWords_1.GIBBERISH_PATTERNS.noVowelSequence.test(text)) {
        const matches = text.match(bannedWords_1.GIBBERISH_PATTERNS.noVowelSequence);
        issues.push({
            type: 'gibberish',
    ...
    if (bannedWords_1.GIBBERISH_PATTERNS.repeatedChar.test(text)) {
        issues.push({
            type: 'gibberish',
    ...
    // Check keyboard mash
    if (bannedWords_1.GIBBERISH_PATTERNS.keyboardMash.test(text)) {
        const matches = text.match(bannedWords_1.GIBBERISH_PATTERNS.keyboardMash);
        issues.push({
            type: 'gibberish',
    ...
    if (letterCount > 10) {
        ...
        if (vowelRatio < bannedWords_1.TEXT_QUALITY_RULES.minVowelRatio) {
            issues.push({
                type: 'gibberish',
...
function checkQuality(text) {
    ...
    if (words.length > 3) {
        ...
        if (repeatedRatio > bannedWords_1.TEXT_QUALITY_RULES.maxRepeatedWordRatio) {
            issues.push({
                type: 'spam',
 ...
 */
function determineAction(issues, strictMode) {
    ...
    if (hasCritical)
        return 'reject';
    if (hasHigh)
        return strictMode ? 'reject' : 'moderate';
    if (hasMedium)
        return strictMode ? 'moderate' : 'flag';
    if (issues.length > 0)
        return 'flag';
    return 'allow';
}
 ...
 */
function isTextValid(text, options) {
