...
app.use(cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'X-CSRF-Token', 'x-otp-token', 'x-app-version']
}));
...
app.get('/health', (req, res) => {
    if (isDbReady()) {
        res.status(200).json({ status: 'ok', db: 'connected', service: 'admin-backend' });
    } else {
        res.status(503).json({ status: 'error', db: 'disconnected', service: 'admin-backend' });
    }
});
